...
{
    CGPoint center = CGPointMake(CGRectGetWidth(self.trackLayer.bounds)/2, CGRectGetHeight(self.trackLayer.bounds)/2);
    CGFloat offset = MAX(self.pointerLength, self.lineWidth / 2.f);
    CGFloat radius = MIN(CGRectGetHeight(self.trackLayer.bounds), CGRectGetWidth(self.trackLayer.bounds)) / 2 - offset;
    UIBezierPath *ring = [UIBezierPath bezierPathWithArcCenter:center radius:radius startAngle:self.startAngle endAngle:self.endAngle clockwise:YES];
...
- (void)updatePointerShape
{
    ...
    [pointer moveToPoint:CGPointMake(CGRectGetWidth(self.pointerLayer.bounds) - self.pointerLength - self.lineWidth / 2.f, CGRectGetHeight(self.pointerLayer.bounds) / 2.f)];
    [pointer addLineToPoint:CGPointMake(CGRectGetWidth(self.pointerLayer.bounds), CGRectGetHeight(self.pointerLayer.bounds) / 2.f)];
...
- (void)setPointerAngle:(CGFloat)pointerAngle animated:(BOOL)animated
{
    ...
    if (animated) {
        ...
        CGFloat midAngle = (MAX(pointerAngle, _pointerAngle) - MIN(pointerAngle, _pointerAngle)) / 2.f + MIN(pointerAngle, _pointerAngle);
